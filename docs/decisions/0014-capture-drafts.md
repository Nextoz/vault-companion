...
**Decision.** A separate IndexedDB store `drafts` keyed by `accountKey` holds `{ kind, text, version, updatedAt }`.
Autosave writes only if the stored version still equals the version it started from and is not older (compare-and-set);
it never recreates a deleted draft. Save is one `readwrite` transaction over `drafts` and `pending`: verify the draft
version, enqueue the envelope, delete the draft. A window whose draft was superseded cannot save that basis. Drafts are
